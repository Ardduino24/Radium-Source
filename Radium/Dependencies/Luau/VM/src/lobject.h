...
// clang-format off
#define CommonHeader \
     VM_SHUFFLE3(Semicolon, uint8_t tt, uint8_t marked, uint8_t memcat)
// clang-format on
...
typedef struct TString
{
    ...
    TStringHashEnc<unsigned int> hash;
    TStringLenEnc<unsigned int> len;

    char data[1]; // string data is allocated right after the header
} TString;

constexpr int Lol56 = offsetof(TString, data);
...
typedef struct Udata
{
    ...
    UdataMetaEnc<struct Table*> metatable;
...
typedef struct Proto
{
    ...
    VM_SHUFFLE4(Semicolon,
    ProtoMember1Enc<TValue*> k,              // constants used by the function
    ProtoMember1Enc<Instruction*> code,      // function bytecode
    ProtoMember1Enc<struct Proto**> p,       // functions defined inside the function
    const Instruction* codeentry);

    void* execdata;
    uintptr_t exectarget;
    VM_SHUFFLE5(Semicolon,
        ProtoMember2Enc<uint8_t*> lineinfo,      // for each instruction, line number as a delta from baseline
        ProtoMember2Enc<int*> abslineinfo,       // baseline line info, one entry for each 1<<linegaplog2 instructions; allocated after lineinfo
        ProtoMember2Enc<struct LocVar*> locvars, // information about local variables
        ProtoMember2Enc<TString**> upvalues,     // upvalue names
        ProtoMember2Enc<TString*> source);

    ProtoDebugnameEnc<TString*> debugname;
    ProtoDebuginsnEnc<uint8_t*> debuginsn; // a copy of code[] array with just opcodes

    ProtoTypeInfoEnc<uint8_t*> typeinfo;

    void* userdata;
    GCObject* gclist;

    VM_SHUFFLE9(Semicolon,
    int sizecode,     // a1
    int sizep,        // a2
    int sizelocvars,  // a3
    int sizeupvalues, // a4
...
typedef struct Closure
{
    ...
    union
    {
        struct
        {
            ClosureFuncEnc<lua_CFunction> f;
            ClosureContEnc<lua_Continuation> cont;
            ClosureDebugnameEnc<const char*> debugname;
            TValue upvals[1];
        } c;

        struct
        {
            ClosureFuncEnc<struct Proto*> p;
            TValue uprefs[1];
        } l;
    };
} Closure;
...
typedef struct Table
{
    ...
    VM_SHUFFLE4(Semicolon,
        TableMemberEnc<struct Table*> metatable,
        TableMemberEnc<TValue*> array,  // array part
        TableMemberEnc<LuaNode*> node,
        GCObject*   gclist);
} Table;
// clang-format on

constexpr int lol = offsetof(Proto, sizek);
...
#define luaO_nilobject reinterpret_cast<TValue*>( Radium::Addresses::Luau::NilObject )

#undef lua_toclosure
#define lua_toclosure(L, idx) clvalue(luaA_toobject(L, idx))
